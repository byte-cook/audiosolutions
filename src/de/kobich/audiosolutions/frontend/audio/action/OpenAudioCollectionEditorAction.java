...
public class OpenAudioCollectionEditorAction extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		OpenAudioCollectionEditorDialog dialog = new OpenAudioCollectionEditorDialog(window.getShell());
		int status = dialog.open();
		if (status == IDialogConstants.OK_ID) {
			final File startDirectory = dialog.getStartDirectory();
			final FileFilter fileFilter = dialog.getFileFilter();
			logger.debug("Add files from dir: " + startDirectory.getPath());
			...
			JFaceThreadRunner runner = new JFaceThreadRunner("Open Audio Collection Editor", window.getShell(), states) {
				...
				protected void run(RunningState state) throws Exception {
					...
					case WORKER_1:
						...
						fileDescriptors = fileDescriptorService.readFiles(startDirectory, fileFilter, super.getProgressMonitor());
						break;
					case UI_2:
						...
						// open editor
						FileOpeningInfo openingInfo = new FileOpeningInfo(startDirectory, fileFilter);
